...
) -> Union[ErrorLogResponse, DetailResponse]:
    """
    Create new error log entry.
    """
    try:
        error_log = crud.create_error_log(db=db, error_log=error_log_in)
        return ErrorLogResponse(data=error_log)
    except Exception as e:
        logger.error(f"Error creating error log: {str(e)}", exc_info=True)
        response.status_code = 500
        return DetailResponse(detail=f"Failed to create error log: {str(e)}")
